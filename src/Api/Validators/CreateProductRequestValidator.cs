...
/// <summary>
/// Ürün oluşturma isteği için validasyon kuralları
/// </summary>
public sealed class CreateProductRequestValidator : AbstractValidator<ProductCreateRequest>
{
    public CreateProductRequestValidator()
    {
        ...
        // Variants validasyonu - ProductVariantDto için basit validasyon
        RuleForEach(x => x.Variants)
            .ChildRules(variant =>
            {
                variant.RuleFor(v => v.VariantName)
                    .NotEmpty().WithMessage("Varyant adı gereklidir")
                ...
                variant.RuleFor(v => v.StockQty)
                    .GreaterThanOrEqualTo(0).WithMessage("Varyant stok miktarı 0 veya daha büyük olmalıdır");
            })
            .When(x => x.Variants.Any());

        // Images validasyonu - ProductImageDto için basit validasyon
        RuleForEach(x => x.Images)
            .ChildRules(image =>
            {
                ...
                image.RuleFor(i => i.DisplayOrder)
                    .GreaterThanOrEqualTo(0).WithMessage("Resim görüntüleme sırası 0 veya daha büyük olmalıdır");
            })
            .When(x => x.Images.Any());
    }
}
